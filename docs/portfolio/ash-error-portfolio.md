...
생성 결과는 그대로 적용하지 않았습니다. 프롬프트 버전과 테이크 번호를 기록하고, 길이·레벨·템포·루프 이음매를 측정한 뒤 청취로 최종 후보를 선택했습니다. 선별한 음원은 페이드 제거, 마디 정렬과 크로스페이드를 거쳐 Ogg/Opus로 변환하고 `AudioDirector`의 스테이지 전환 이벤트에 연결했습니다.

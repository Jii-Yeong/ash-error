...
<p align="center">
  <img src="./assets/title-screen.webp" alt="ASH//ERROR 타이틀 화면" width="900" />
</p>
...
| 게임 엔진   | Phaser 4                                                                                 |
| UI          | React 19, Zustand                                                                        |
| 언어와 빌드 | TypeScript, Vite, pnpm workspace                                                         |
| 검증        | Vitest, Playwright E2E, TypeScript, lint, build                                          |
| 작업 범위   | 게임 기획, 전투 및 이동 구현, 레벨 디자인, 아트 제작 파이프라인, 오디오 통합, QA, 문서화 |

## 핵심 경험

```text
방 진입 → 전투 공간 봉쇄 → 역할이 다른 적 조합 대응
→ 무기 획득과 교체 → 방 클리어 → 보스 패턴 학습 → 다음 스테이지
```
...
흰색, 금색, 하늘색과 후광, 날개, 십자 형태를 사용해 인공적인 구원의 이미지를 만들었습니다. 밝고 개방된 배경과 달리 실제 전투는 자유 비행과 방사 탄막으로 가장 높은 압박을 제공하며, 최종 보스의 흰색과 금색 실루엣이 묻히지 않도록 하늘색 명암으로 공간의 깊이를 나눴습니다.

## 생성형 음악을 스테이지 정서로 연결

스테이지 음악은 Gemini 웹 앱의 Lyria로 후보를 생성했습니다. 분위기만 묘사하지 않고 조성, BPM, 악기, 음역, 공간의 잔향과 반복 구조를 프롬프트에 고정하고, 결과가 게임의 전투 밀도와 세계관에 맞을 때까지 버전과 테이크를 나눠 비교했습니다.

| THE UNDERGROUND                                                                                                       | HELL                                                                                                      | THE RETURN                                                                                                          |
| --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| <img src="./assets/music-generation-underground.jpg" alt="Gemini에서 생성한 Just After They Left 트랙" width="300" /> | <img src="./assets/music-generation-inferno.jpg" alt="Gemini에서 생성한 Ten Ton Loom 트랙" width="300" /> | <img src="./assets/music-generation-return-final.jpg" alt="Gemini에서 생성한 Surgical Meridian 트랙" width="300" /> |
| `Just After They Left` (테이크 19)                                                                                    | `Ten Ton Loom` (테이크 20)                                                                                | `Surgical Meridian` (테이크 67)                                                                                     |

지하는 저음 현악과 긴 잔향으로 사람이 떠난 콘크리트 공간의 상실감을 만들었습니다. 지옥은 같은 어두운 소재를 빠른 드럼과 왜곡 기타, 현악의 반복으로 밀어붙였습니다. 인공 낙원은 밝은 장조를 유지하면서 하프 오스티나토와 글라스 하모니카, 기계적으로 고정된 화성을 사용해 아름답지만 무심한 시설의 인상을 만들었습니다.

생성 결과는 그대로 적용하지 않았습니다. 프롬프트 버전과 테이크 번호를 기록하고, 길이, 레벨, 템포와 루프 이음매를 측정한 뒤 청취로 최종 후보를 선택했습니다. 선별한 음원은 페이드 제거, 마디 정렬과 크로스페이드를 거쳐 Ogg/Opus로 변환하고 `AudioDirector`의 스테이지 전환 이벤트에 연결했습니다.
...
달리기 애니메이션은 여섯 프레임의 발 순서와 체중 이동을 먼저 정했습니다. 모든 프레임에서 머리 윗선, 눈높이, 손과 총기 위치, 발바닥 기준선을 유지하고, 착지와 공중 프레임의 높이 차이만으로 속도감을 만들었습니다.
...
| Aseprite 프레임과 레이어 검수                                                                                                             | 실제 게임의 근접 전투                                                                                                     |
| ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| <img src="./assets/aseprite-player-animation-workflow.webp" alt="Aseprite에서 주인공 애니메이션을 편집하고 미리 보는 화면" width="440" /> | <img src="./assets/player-melee-gameplay.webp" alt="미래 도시에서 주인공과 근접형 적이 대치하는 게임 화면" width="440" /> |

Aseprite의 태그, 레이어와 미리보기 창으로 대기, 달리기, 점프와 사망 상태의 연결과 기준점을 확인했습니다. 완성한 스프라이트는 실제 배경과 적 사이에서 크기, 실루엣과 공격 자세가 읽히는지 다시 검수했습니다.
...
최종 보스는 흰색과 금색 장갑, 하늘색 발광부, 거대한 후광과 기계 날개를 사용합니다. 성스럽고 깨끗한 외형과 플레이어를 제거하려는 기능이 충돌하도록 설계했습니다. 후광, 날개, 단안 코어를 각각 공격 패턴의 중심 장치로 사용해 자세만 보아도 다음 위협을 예측할 수 있게 했습니다.
...
- 이동과 공격 애니메이션의 자연스러운 연결
- 팔과 다리 관절, 기계 부품의 움직임 검토
...
| ----------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| 플레이어          | 이동과 공격, 피격 및 사망 피드백           | 대기, 이동, 공격, 피격, 쓰러짐, 앞으로 엎드림                                                                                         |
...
안드로이드와 기계형 적은 프레임마다 팔과 다리 관절의 위치와 접히는 방향, 케이블 연결점, 무기 장착 위치, 등과 팔의 장비 개수를 비교했습니다. AI로 여러 자세를 생성하면 부품이 사라지거나 추가되고 손이 다른 구조로 바뀌는 경우가 있어, 원본 스프라이트를 기준 레이어로 두고 직접 수정했습니다.
...
- 스테이지별 팔레트를 적용하되 충돌과 도달성 규칙은 공통으로 유지
...
- Phaser: 월드, 물리, 플레이어와 적, 투사체, 보스 패턴과 화면 연출
...
손과 총을 이미지 생성 단계에서 합치자 양쪽 원본 픽셀이 다시 그려졌습니다. 그래서 앞팔과 손, 총, 뒷팔을 별도 스프라이트로 나누고 런타임에서 같은 각도로 회전시키는 구조로 변경했습니다. 시각 결과가 게임 규칙에 맞지 않을 때 코드를 억지로 보정하기보다 에셋과 구조의 책임을 다시 나눈 사례입니다.
...
4스테이지 보스 처치 후 보스 소멸, 화면 파괴, 다음 스테이지 음악이 서로 다른 시스템에 흩어져 있었습니다. 이를 `보스 처치 → 쓰러짐과 소멸 → 화면 유지 → 화면 파괴 → 완료 후 BGM 시작` 순서로 정리하고 이벤트와 테스트로 고정했습니다.

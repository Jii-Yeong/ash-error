...
| 샘플레이트 | 소스는 44.1kHz. **저장본은 48kHz다** — Opus는 내부적으로 48kHz로만 동작해 컨테이너가 44.1kHz를 담지 못한다 |
| 채널 | SFX 모노, BGM 스테레오 |
...
## 스테이지 3~5 보스 패턴 큐 (자체 합성)

25개 전부 `tools/make-boss-sfx.mjs`로 생성했다. 시드가 고정돼 있어 재생성하면 지금
저장소에 있는 파일과 **바이트 단위로 같다** — 렌더된 오디오가 아니라 레시피를 저장소가
들고 있다는 뜻이고, 몇 달 뒤에도 믹스를 다시 만질 수 있다.

```
node tools/make-boss-sfx.mjs <출력폴더>     # wav 25개
```

공용 신스 primitive는 `tools/audio-dsp.mjs`에 있고 `make-gunshot.mjs`도 같은 모듈을
쓴다. (분리 전후로 총소리 5개 파일의 md5가 동일한 것을 확인했다.)

### 스테이지별 음향 정체성

큐가 어느 스테이지 것인지 **소리만 듣고 구분되지 않으면 그 스테이지 것이 아니다**를
기준으로 나눴다.

| 스테이지 | 보스 | 재질 | 음높이 |
| --- | --- | --- | --- |
| 3 | 정화 집행자 | 유압·공압. 압력이 차고, 터지고, 금속이 콘크리트에 떨어진다 | 없음(전부 노이즈) |
| 4 | 지옥 집행자 | 암석·마그마. 3보다 낮고 거칠며, 기계 히스 대신 균열 트랜지언트 | 없음(전부 노이즈) |
| 5 | 귀환한 설계자 | 종·성가 | **게임에서 유일하게 음정이 있는 큐** |

5스테이지만 음정을 허용한 게 핵심이다. 최종 보스는 이 세계 위쪽에 속한 것처럼 들려야
하는 유일한 대상이다. 음높이는 **D**에 맞췄다 — BGM 설계상 `title`·`city`가 D major,
`alley`가 D minor이므로, 근음과 5도만 쓰면 `bgm-return`이 나중에 어느 모드로 쓰이든
조성이 틀릴 수 없다.

### 큐 목록

전부 텔레그래프 → 임팩트 순으로 짝을 이루고, **경고음이 항상 더 작고 둔하다.** 그래야
플레이어가 해결음을 사건으로 듣는다.

| 에셋 키 | 패턴 시점 | 방향성 |
| --- | --- | --- |
| `sfx-stage3-boss-slam-warn` | 도약 예고(900ms) | 유압 축압. 텔레그래프보다 짧게 끝나 한 박 정적을 남긴다 |
| `sfx-stage3-boss-slam-leap` | 도약 | 공압 배출. 짧아야 갑작스럽게 들린다 |
| `sfx-stage3-boss-slam-impact` | 착지 | 스테이지 3에서 가장 큰 큐 |
| `sfx-stage3-boss-shockwave` | 압력파 2발 | 임팩트보다 낮게 깔아 "여파"로 들리게. 좌우 동시라 큐는 1회만 |
| `sfx-stage3-boss-vacuum-start` | 흡입 예고(700ms) | 터빈 스핀업 |
| `sfx-stage3-boss-vacuum-loop` | 흡입 지속 | **심리스 루프.** 양끝을 페이드하지 않는다 |
| `sfx-stage3-boss-vacuum-end` | 흡입 종료 | start의 역재생 격. 쌍으로 구간을 닫는다 |
| `sfx-stage4-boss-rupture-warn` | 분출 예고(700ms) | 지면 균열 |
| `sfx-stage4-boss-rupture-erupt` | 분출(3회, 250ms 간격) | 꼬리를 간격보다 짧게 잘라 3번째가 뭉개지지 않게 |
| `sfx-stage4-boss-charge-warn` | 돌진 예고(850ms) | 압력 축적 |
| `sfx-stage4-boss-charge-rush` | 돌진(600ms) | 통과하며 음이 떨어진다 |
| `sfx-stage4-boss-charge-impact` | 벽 충돌 | 뒤가 플레이어의 딜 타임이므로 **보스가 실패한 소리**로 붕괴시킨다 |
| `sfx-stage4-boss-shard-fall` | 파편 낙하(650ms) | 조용하게. 착탄이 사건이다. 4레인이어도 큐는 1회 |
| `sfx-stage4-boss-shard-impact` | 파편 착탄 + 마그마 | 4레인이 같은 프레임에 떨어지므로 `minInterval: 90`으로 1발로 합친다 |
| `sfx-stage4-boss-phase-shift` | 2페이즈 전환 | 스테이지 4 최대 큐 |
| `sfx-stage5-boss-halo-warn` | 후광 충전(700ms) | 성가풍 상승. 트랜지언트 없음 |
| `sfx-stage5-boss-halo-ring` | 링 탄막 발사 | 때린 소리. 비정수배 배음이라 음이 아니라 종으로 들린다 |
| `sfx-stage5-boss-wings-warn` | 날개 전개(550ms) | 공기 + 부드러운 배음 하나 |
| `sfx-stage5-boss-wings-fan` | 부채꼴 발사 | 3연속이라 halo보다 밝고 짧게. 2문 동시 발사도 큐 1회 |
| `sfx-stage5-boss-eye-track` | 눈 추적(800ms) | **도달하지 않는 상승.** 해결되지 않는 상승이 가장 싸게 플레이어를 움직인다 |
| `sfx-stage5-boss-eye-lock` | 락온(350ms) | 으뜸음 위 트라이톤. 이 조성이 흡수할 수 없는 유일한 음정이라 경보로 들린다 |
| `sfx-stage5-boss-eye-orb` | 심판구 작렬 | 종 + 파열 |
| `sfx-stage5-boss-phase-shift` | 2페이즈 전환 | 성가가 처음으로 제대로 도착한다 |
| `sfx-stage5-boss-salvation` | 거짓 구원 | 게임에서 가장 긴 큐이자 **유일하게 아름다워야 하는 큐.** D major 3화음으로 시작해 스스로 반음쯤 내려앉는다 — 제시된 구원이 듣는 동안 부패한다 |
| `sfx-stage5-boss-core-exposed` | 코어 노출 | 옥타브 위쪽을 내려 비틀었다. 안에 머무르기 불쾌해야 딜 타임을 쏘는 데 쓴다 |

### 지속음은 한 곳에서 끊는다

`vacuum-loop`은 `loop: true`로 재생되므로 **끝내 주는 쪽이 없으면 영원히 남는다.** 이걸
각 보스가 알아서 emit하게 두면, 보스의 `updateCombat`이 더는 호출되지 않는 상황(사망,
일시정지, 어그로 이탈)에서 정지 신호가 나갈 자리가 사라진다.

그래서 지속음은 `audioConfig.ts`의 `SUSTAINED_SFX`에 등록하고 `AudioDirector`가
한 곳에서 끊는다 — `stage-changed`, 타이틀 복귀, **`phase-changed: 'dead'`**. 일시정지는
정지가 아니라 `pause()`/`resume()`이라 재개하면 전투 중간부터 이어진다. 스테이지 2 스캔
루프도 같은 경로로 옮겼다.

> **포맷.** 25개 전부 `.ogg`(Opus) 모노다. 합계 **179KB**.

**스테이지 1~2 보스 큐는 아직 이 대장에 없다.** PR 13에서 코드에는 들어갔지만 문서에
반영되지 않았다. 문서와 코드가 같아야 한다는 이 파일의 전제가 그만큼 깨져 있다.

## SFX 확장자를 `.ogg`로 통일했다

규격 첫 줄이 "`.ogg` 컨테이너에 Opus, BGM·SFX 전부 같다"인데 `sfx/`에는 `.ogg` 36개,
`.wav` 19개, `.mp3` 16개가 섞여 있었다. 35개를 Opus로 다시 인코딩하고 원본을 지웠다.

| | 변환 전 | 변환 후 |
| --- | --- | --- |
| `.wav` 19개 | 1,581KB | 116KB |
| `.mp3` 16개 | 50KB | 39KB |
| **`sfx/` 전량** | **1,862KB** | **386KB** |

부팅 배치가 전량 블로킹 로드이므로 이 1.5MB가 그대로 타이틀 화면 대기 시간이었다.
스테이지 1 보스 레이저 3개만 999KB였고 지금은 55KB다.

비트레이트는 모노 64k, 스테레오 96k. 채널 수와 길이는 소스 그대로 두었다.

### 확인한 것

- **디코딩 길이가 35개 전부 2ms 이내로 보존된다.** `ffprobe`가 보고하는 컨테이너 길이는
  전 파일에서 6ms 늘어나 보이는데, 이건 Opus 마지막 프레임 패딩이지 디코더가 돌려주는
  샘플 수가 아니다. 애니메이션에 타이밍이 물린 큐가 밀리면 안 되므로 **컨테이너 길이가
  아니라 디코딩 결과로 재야 한다.**
- **`stage2_boss_scan_loop`의 이음매가 살아 있다.** 심리스 루프라 손실 압축이 이음매를
  깨면 매 바퀴 클릭이 난다. 인코딩 후 랩어라운드 불연속은 0.00994로, 평균 샘플 간
  변화량(0.02824)보다 **작다.** 프레임 수(57600)도 그대로다.

### 남은 부채

발소리 16개는 mp3에서 다시 인코딩한 것이라 **세대 손실이 한 번 더 얹혔다.** 짧은
노이즈 버스트라 실용상 문제는 없지만, 무손실 마스터가 남아 있다면 거기서 다시 뽑는
편이 낫다. 이 16개는 **라이선스 대장에도 없다** — 직접 생성이라면 서비스명·접근
경로·약관상 이용 범위를 기록해야 한다.

## SFX도 스테이지 단위로 지연 로드한다

포맷을 정리해 386KB가 됐어도 **전량이 블로킹 부팅 배치에 들어가는 구조**는 그대로였다.
도달하지도 않을 5스테이지 보스 큐까지 타이틀 화면 앞에서 기다릴 이유가 없다.

음악이 이미 쓰던 정책을 효과음에도 그대로 적용했다 — `DEFERRED_SFX_BY_STAGE`에
스테이지 전용 큐를 묶고, `AudioDirector`가 **지금 스테이지와 그 다음 하나**를 미리
가져온다.
...
부팅 SFX 페이로드는 **1,723KB → 78KB**가 됐다. 남는 것은 어느 스테이지에서도 나는
큐뿐이다 — 무기 4종, 피격·격파, 플레이어, 방 잠금/해제, 방어 피격.

**한 스테이지 앞서 받는 것이 핵심이다.** 큐는 순간음이라 아직 도착하지 않았으면 큐에
쌓이지 않고 그냥 버려진다(늦게 나는 총소리는 버그로 들린다). 실제로 필요해지는 시점보다
2~3분 앞서 요청이 나가야 하는 이유다. 타이틀 화면에서 1스테이지 몫을 미리 받는 것도
같은 이유다.

새 스테이지 큐를 추가하면서 이 집합에 넣는 것을 잊으면 조용히 부팅 페이로드로
들어가고 **아무것도 깨지지 않는다.** 그래서 이름이 `sfx-stage<N>-`으로 시작하는 큐는
전부 지연 대상이어야 한다는 것을 테스트로 박아 뒀다.

## 후속 확장 시 검토할 큐

현재 핵심 BGM, 스테이지 1~5 보스 SFX, 발소리, 엔딩 전환음은 연결돼 있다. 아래 항목은 새 기능 또는 연출을 추가할 때만 검토한다.

| 시점 | 필요한 사운드 |
| --- | --- |
| 스테이지 3~5 | 적용 완료 — `bgm-underground`, `bgm-inferno`, `bgm-return` |
